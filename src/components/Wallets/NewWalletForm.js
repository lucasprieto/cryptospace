...
const pMap = platforms.reduce((obj, el) => {
  obj[el.value] = el;
  obj[el.value].chip = (
    <Chip avatar={<ImageAvatar chip alt={el.name} src={`/crypto-assets/${el.icon}`} />} label={el.currency} />
  );
  return obj;
}, {});
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} md={3}>
          <FormControl fullWidth>
            <InputLabel htmlFor="platform" id="platform-lbl">
              Platform
            </InputLabel>
            <Controller
              name="platform"
        ...
        <Grid item xs={12} md={3}>
          <Grid container spacing={1} alignItems="flex-end" className={classes.balanceGrid}>
            {pMap[platform] && <Grid item>{pMap[platform].chip}</Grid>}
            <Grid item className={classes.balance}>
              <Controller
                name="balance"
                control={control}
                render={({ field }) => <TextField fullWidth disabled label="Balance" {...field} />}
              />
            </Grid>
          </Grid>
        </Grid>
